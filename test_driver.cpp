...
int run_anns(int argc, char **argv)
{
	...
	uint32_t* query_filters=(uint32_t*)malloc(sizeof(uint32_t)*numQueries);
	if(query_filters==NULL){
		printf("Error.. Malloc failed for query filters file");
		return -1;
	}

	string query_filters_filename= string(argv[8]);
	ifstream in8(query_filters_filename);
	if (!in8.is_open())
	{
		printf("Error.. Could not open the Query Filters File: %s\n", query_filters_filename.c_str());
		return -1;
	}
	...
	size_t counter=0;//no of labels

	ifstream in9(datapoints_filter_filename);
	if(!in9.is_open()){
		printf("Error: Could not open the datapoints labels file %s\n",datapoints_filter_filename.c_str());
	}
	...
	uint32_t* datapoints_filter=(uint32_t*)malloc(sizeof(uint32_t)*counter);
	int* label_offsets= (int*)malloc(sizeof(int)*no_points);
	int* label_counts= (int*)malloc(sizeof(int)*no_points);

	ifstream in9i(datapoints_filter_filename);
	int cnt=0;
	...
	string labels_to_medoid_filename= string(argv[10]);

	ifstream in10(labels_to_medoid_filename);
	if(!in10.is_open()){
		printf("Error: Could not open the labels to medoids file %s\n",labels_to_medoid_filename.c_str());
	}
...
//            if (nIter == 0)
			...
			objBANG.bang_alloc(numQueries,query_filters,datapoints_filter,label_counts,label_offsets,labels_to_medoid_map);
...
int main(int argc, char **argv)
{
	...
	if (argc < 11)
	{
		...
		<query file> <GroundTruth File> <NumQueries> <recall parameter k> <data type : uint8, int8 or float> <dist funct: l2 or mips> <query filter file> <datapoints filter file> <labels_medoids file>"
			 << endl;
		exit(1);
	}
