...
typedef struct _HostInstance
{
	...
	std::unordered_map<uint32_t,unsigned> labels_to_medoid_map;

	uint32_t* datapoints_filter;
	int* label_counts;
	int* label_offsets;
	
	uint32_t* query_filters;
...
public:
    ...
    void bang_alloc(int numQueries,uint32_t* query_filters,uint32_t* datapoints_filter,int* label_counts,int* label_offsets,std::unordered_map<uint32_t,unsigned> & labels_to_medoid_map);
