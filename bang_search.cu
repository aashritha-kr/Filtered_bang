...
#define MAX_R 32 // Degree bound of the graph. compile-time definition to enable usage in arrays. Update it to match m_objInputData.R
	...
	for(unsigned ii=0; ii < numQueries; ++ii ) {
		...
		for(unsigned i=0;i<med_degree;++i){
			...
			for(int j=0;j<count;++j){
				if(m_objHostInst.datapoints_filter[offset+j]==query_label){
 ...
 */
								...
								unsigned R)
 {
	...
	for(unsigned ii=0; ii < numNeighbors; ++ii) {
		if(d_neighborsDist_query[offset + ii] < dist && d_neighbors[offset+ii]!= d_L2ParentIds[queryID]){
			index=offset+ii;
