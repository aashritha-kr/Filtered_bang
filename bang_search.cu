...
template<typename T>
void BANGSearch<T>::bang_alloc(int numQueries,uint32_t* query_filters,uint32_t* datapoints_filter,int* label_counts,int* label_offsets,std::unordered_map<uint32_t,unsigned> & labels_to_medoid_map)
{
	BANGSearchInner<T>* pobjBangInner = static_cast<BANGSearchInner<T>*>(m_pImpl);
	pobjBangInner->bang_alloc(numQueries,query_filters,datapoints_filter, label_counts, label_offsets,labels_to_medoid_map);
}
...
template<typename T>
void BANGSearchInner<T>::bang_alloc(int numQueries,uint32_t* query_filters,uint32_t* datapoints_filter,int* label_counts,int* label_offsets,std::unordered_map<uint32_t,unsigned>& labels_to_medoid_map)
{
	...
	// Host Memory Allocation
 	m_objHostInst.query_filters = query_filters;
	m_objHostInst.labels_to_medoid_map=labels_to_medoid_map;
	m_objHostInst.label_offsets=label_offsets;
	m_objHostInst.label_counts=label_counts;
	m_objHostInst.datapoints_filter=datapoints_filter;
...
void BANGSearchInner<T>::bang_init(int numQueries)
{
	...
	{
		uint32_t query_label=m_objHostInst.query_filters[i];
		unsigned med=m_objHostInst.labels_to_medoid_map[query_label];
		L2ParentIds[i] = med;
...
// ToDo: Fetching the neighbours of Medoid and transferring them to GPU can be moved to Load Phase
	...
	for(unsigned ii=0; ii < numQueries; ++ii ) {
		uint32_t query_label=m_objHostInst.query_filters[ii];
		unsigned med=m_objHostInst.labels_to_medoid_map[query_label];

        unsigned* puNumNeighbours = (unsigned*)( m_objInputData.pIndex + ((m_objInputData.ullIndex_Entry_LEN * med) + (m_objInputData.D*sizeof(T))) );
		unsigned med_degree=*puNumNeighbours;
		unsigned* puNeighbor=puNumNeighbours +1;

		m_objHostInst.neighbors[ii * uMaxNeighbors] = med;
		unsigned numNeighbors = 1;
		for(unsigned i=0;i<med_degree;++i){
			// m_objHostInst.neighbors[ii * uMaxNeighbors + i + 1] = puNeighbor[i];
            // numNeighbors++;
			unsigned neighbor_id=puNeighbor[i];
			int offset=m_objHostInst.label_offsets[neighbor_id];
			int count=m_objHostInst.label_counts[neighbor_id];

			for(int j=0;j<count;++j){
				if(m_objHostInst.datapoints_filter[offset+j]==query_label||m_objHostInst.datapoints_filter[offset+j]==0){
					m_objHostInst.neighbors[ii*uMaxNeighbors+numNeighbors]=neighbor_id;
					numNeighbors++;
		...
		// if (ii < 5) { // DEBUG: Print for first 5 queries
		// 	printf("DEBUG bang_init: query %d, query_label %u, medoid %u, initial_neighbors %u, filtered_neighbors %u\n", ii, query_label, med, med_degree, numNeighbors);
		// }

		// if(ii==0)
		// {
		// std::cout<<"The no of filtered neighbors for the medoid"<<med<<" are "<<numNeighbors<<std::endl;
		// std::cout<<"size of the vector is "<<vec.size()<<std::endl;
		// for(auto i : vec){
		// 	std::cout<<"The filtered neighbor that matches the label are "<<i;
		// }
		// std::cout<<std::endl;
		// }
		...
		memcpy(FPSetCoordsList + ((iter-1) * m_objHostInst.FPSetCoords_rowsize) + (ii*m_objHostInst.FPSetCoords_size),
                m_objInputData.pIndex + (m_objInputData.ullIndex_Entry_LEN * med) ,
		...
		#pragma omp parallel
		{
			...
			for(unsigned ii=CPUthreadno; ii < numQueries; ii = ii + m_objHostInst.numCPUthreads)
			{
				...
				for(unsigned jj = 1; jj <= numParents; ++jj) //numParents is 1 today
				{
					...
					unsigned no_initial_neighbors = *puNumNeighbours;
                    unsigned* initial_neighbors = puNumNeighbours + 1;

					unsigned filtered_neighbors[MAX_R+1]; 
                    unsigned filtered_neighbor_count = 0;

					for(int i=0;i<no_initial_neighbors;++i){

						unsigned neighbor_id=initial_neighbors[i];
						...
						for(int j=0;j<count;++j){
							if(m_objHostInst.datapoints_filter[offset+j]==query_label||m_objHostInst.datapoints_filter[offset+j]==0){
								filtered_neighbors[filtered_neighbor_count]=neighbor_id;
					...
					memcpy(m_objHostInst.neighbors+offset_neighbors,filtered_neighbors, filtered_neighbor_count * sizeof(unsigned));
					m_objHostInst.numNeighbors_query[ii] = filtered_neighbor_count;

					// if (ii < 5 && iter < 3) { // DEBUG: Print for first 5 queries and first 2 iterations
					// 	printf("DEBUG bang_query: iter %u, query %u, parent %u, initial_neighbors %u, filtered_neighbors %u\n", iter, ii, curreParent, no_initial_neighbors, filtered_neighbor_count);
					// }
				}
			}
		}
 ...
 */
								...
								unsigned R)
 {
	...
	for(unsigned ii=0; ii < numNeighbors; ++ii) {
		if(d_neighborsDist_query[offset + ii] < dist && d_neighbors[offset+ii]!= 8927){
			index=offset+ii;
